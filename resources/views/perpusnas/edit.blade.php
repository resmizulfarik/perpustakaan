...
<head>
    ...
    <title>Edit Layanan Perpusnas</title>
        ...
        <div class="col-md-7">
            <div class="card card-form">
                <div class="card-header bg-white py-3">
                    <h4 class="mb-0 fw-bold"><i class="fas fa-edit me-2"></i>Edit Layanan Jaringan</h4>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('perpusnas.update', $perpusna->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-3">
                            <label class="form-label fw-bold">Nama Layanan</label>
                            <input type="text" name="nama_layanan" class="form-control" value="{{ $perpusna->nama_layanan }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">URL / Link Website</label>
                            <input type="url" name="url_link" class="form-control" value="{{ $perpusna->url_link }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">Logo Saat Ini</label><br>
                            <img src="{{ asset('images/perpusnas/' . $perpusna->logo) }}" width="80" class="mb-2 rounded">
                            <input type="file" name="logo" class="form-control">
                            <small class="text-muted text-italic">*Kosongkan jika tidak ingin ganti logo</small>
                        </div>
