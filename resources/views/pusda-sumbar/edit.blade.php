...
<head>
    ...
    <title>Edit Layanan Pusda Sumbar</title>
        ...
        <div class="col-md-7">
            <div class="card card-form">
                <div class="card-header bg-white py-3">
                    <h4 class="mb-0 fw-bold"><i class="fas fa-edit me-2 text-danger"></i>Edit Layanan Pusda Sumbar</h4>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('pusda-sumbar.update', $pusdaSumbar->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-3">
                            <label class="form-label fw-bold">Nama Layanan</label>
                            <input type="text" name="nama_layanan" class="form-control" value="{{ $pusdaSumbar->nama_layanan }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">URL / Link Website</label>
                            <input type="url" name="url_link" class="form-control" value="{{ $pusdaSumbar->url_link }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">Deskripsi Singkat</label>
                            <textarea name="deskripsi" class="form-control" rows="3">{{ $pusdaSumbar->deskripsi }}</textarea>
                        </div>
