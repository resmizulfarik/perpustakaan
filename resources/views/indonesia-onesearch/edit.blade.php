...
<head>
    ...
    <title>Edit Indonesia OneSearch</title>
        ...
        <div class="col-md-7">
            <div class="card card-form">
                <div class="card-header bg-white py-3">
                    <h4 class="mb-0 fw-bold text-dark"><i class="fas fa-edit me-2"></i>Edit Layanan IOS</h4>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('indonesia-onesearch.update', $onesearch->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-3">
                            <label class="form-label fw-bold">Nama Layanan</label>
                            <input type="text" name="nama_layanan" class="form-control" value="{{ $onesearch->nama_layanan }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">URL / Link Website</label>
                            <input type="url" name="url_link" class="form-control" value="{{ $onesearch->url_link }}" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">Deskripsi</label>
                            <textarea name="deskripsi" class="form-control" rows="3">{{ $onesearch->deskripsi }}</textarea>
                        </div>
